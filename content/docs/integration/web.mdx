...
<Callout title="Official SDKs are available" type="info">
  Use the official **Node.js SDK** in JavaScript and TypeScript backends. The browser still calls your backend proxy—the SDK does not make exposing an API key in client-side code safe.
</Callout>
