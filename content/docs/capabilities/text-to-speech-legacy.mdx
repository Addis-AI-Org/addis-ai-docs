...
This page preserves the original Voice 1 Base64 and streaming workflow for existing applications. New integrations should use [Text-to-Speech with Addis Voices 2](/docs/capabilities/text-to-speech).
</Callout>
...
<TypeTable
  type={{
    ...
    'language': {
      description: 'Language code. "am" (Amharic) or "om" (Afaan Oromo).',
      type: 'string',
      required: true
    },
    'stream': {
...
Addis Voices 2 adds catalog discovery, previews, estimates, durable clips, idempotency, and minute-based billing. Start with the [Addis Voices 2 Text-to-Speech guide](/docs/capabilities/text-to-speech), select a current catalog voice such as `am-hamen`, and migrate one existing use case at a time. The current guide includes a [field-by-field migration summary](/docs/capabilities/text-to-speech#migrate-from-legacy-text-to-speech).
